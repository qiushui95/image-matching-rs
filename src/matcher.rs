...
impl ImageMatcher {
    ...
    pub fn matching(
        ...
        match &self.template_data {
            TemplateData::FFT { data } => Ok(Self::perform_fft_matching(data, image, threshold)),
            TemplateData::Segmented { data } => Ok(Self::perform_segmented_matching(data, image, threshold)),
        }
    }
    ...
    fn calculate_fft_correlations(
        ...
        // 按相关系数降序排序并转换为结果格式
        Self::sort_and_convert_results(found_points, template_data.template_width, template_data.template_height)
    }
    ...
    fn calculate_single_fft_correlation(
        ...
        // 计算分子（从FFT结果获取，需要归一化）
        let numerator = fft_result[y as usize * template_data.padded_size as usize + x as usize].re as f64
        ...
        let denominator = (template_data.template_sum_squared_deviations as f64 * image_sum_squared_deviations).sqrt();
    ...
    /// # 返回值
    /// 调整后的阈值结构
    fn calculate_adjusted_thresholds(template_data: &SegmentedTemplateData, threshold: f32) -> AdjustedThresholds {
        AdjustedThresholds {
    ...
    fn calculate_segmented_correlations(
        ...
        // 按相关系数降序排序并转换为结果格式
        Self::sort_and_convert_results(found_points, template_width, template_height)
    }
    ...
    fn calculate_segment_correlation(
        ...
        for &(seg_x, seg_y, seg_width, seg_height, segment_mean) in segments {
            let region_sum = Self::sum_region(
                image_integral,
                x + seg_x,
                y + seg_y,
                seg_width,
    ...
    /// # 返回值
    /// 排序后的匹配结果列表
    fn sort_and_convert_results<T: Into<f32> + Copy>(
        mut found_points: Vec<(u32, u32, T)>,
        template_width: u32,
        template_height: u32,
    ) -> Vec<MatcherResult> {
        // 按相关系数降序排序
        found_points.sort_by(|a, b| {
            let corr_a: f32 = a.2.into();
            let corr_b: f32 = b.2.into();
    ...
    /// # 返回值
    /// 合并后的分段列表
    fn merge_similar_segments(mut segments: Vec<(u32, u32, u32, u32, f32)>) -> Vec<(u32, u32, u32, u32, f32)> {
